...
$siteEmail = site_setting('contact_email', 'support@greenerry.test');
$sitePhone = site_setting('contact_phone', '+351 900 000 000');
$instagramUrl = site_setting('instagram_url', '#');
$xUrl = site_setting('x_url', '#');
$currentYear = (int)date('Y');
foreach (['instagramUrl', 'xUrl'] as $socialUrlVar) {
    if ($$socialUrlVar !== '' && $$socialUrlVar !== '#' && !preg_match('#^[a-z][a-z0-9+.-]*://#i', $$socialUrlVar) && !str_starts_with($$socialUrlVar, 'mailto:') && !str_starts_with($$socialUrlVar, 'tel:')) {
        $$socialUrlVar = 'https://' . ltrim($$socialUrlVar, '/');
    }
}
?>
...
<?php endif; ?>
    ...
    <div class="wrap">
      <div class="foot-grid">
        ...
        <div class="foot-col">
          ...
          <a href="<?= $_base ?>/pages/artist_dashboard.php" data-t="nav_artist_side">Artist side</a>
        </div>
        <div class="foot-col">
          <h5 data-t="foot_contact">Contact</h5>
          <a href="mailto:<?= h($siteEmail) ?>"><?= h($siteEmail) ?></a>
          <a href="tel:<?= h(preg_replace('/\s+/', '', $sitePhone)) ?>"><?= h($sitePhone) ?></a>
        </div>
        <div class="foot-col">
          <h5 data-t="foot_legal">Legal</h5>
          <button type="button" class="foot-link-button" data-cookie-settings data-t="cookies_settings">Cookie settings</button>
          <span data-t="foot_privacy_note">Educational PAP project with privacy-aware demo data.</span>
        </div>
      </div>
      <div class="foot-btm">
        <p class="foot-copy">&copy; <?= $currentYear ?> Greenerry. <span data-t="foot_rights">All rights reserved.</span></p>
        <div class="foot-socials">
          <a href="<?= h($instagramUrl) ?>" class="soc" aria-label="Instagram" target="_blank" rel="noopener noreferrer"><svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg></a>
          <a href="<?= h($xUrl) ?>" class="soc" aria-label="X" target="_blank" rel="noopener noreferrer"><svg width="12" height="12" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
        </div>
      </div>
    </div>
  </footer>
  ...
  <section class="cookie-banner" data-cookie-banner hidden>
    <div class="cookie-banner-copy">
      <strong data-t="cookies_title">Cookies on Greenerry</strong>
      <p data-t="cookies_text">We use essential cookies to keep login, language, theme and cart working. Optional cookies help remember preferences during the demo.</p>
    </div>
    <div class="cookie-banner-actions">
      <button type="button" class="btn btn-ghost btn-sm" data-cookie-reject data-t="cookies_reject">Reject optional</button>
      <button type="button" class="btn btn-dark btn-sm" data-cookie-accept data-t="cookies_accept">Accept</button>
    </div>
  </section>
...
<div class="player-bar" id="player-bar">
  ...
  <div class="pb-center">
    <div class="pb-ctrls">
      ...
      <button class="pb-btn" id="pb-loop" title="Loop">
        <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M4.5 3A3.5 3.5 0 0 0 1 6.5v1A3.5 3.5 0 0 0 4.5 11H10l-1.15 1.15a.5.5 0 0 0 .7.7l2-2a.5.5 0 0 0 0-.7l-2-2a.5.5 0 1 0-.7.7L10 10H4.5A2.5 2.5 0 0 1 2 7.5v-1A2.5 2.5 0 0 1 4.5 4H6a.5.5 0 0 0 0-1H4.5zm7 10A3.5 3.5 0 0 0 15 9.5v-1A3.5 3.5 0 0 0 11.5 5H6l1.15-1.15a.5.5 0 1 0-.7-.7l-2 2a.5.5 0 0 0 0 .7l2 2a.5.5 0 0 0 .7-.7L6 6h5.5A2.5 2.5 0 0 1 14 8.5v1a2.5 2.5 0 0 1-2.5 2.5H10a.5.5 0 0 0 0 1h1.5z"/></svg>
      </button>
